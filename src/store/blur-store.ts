...
interface BlurState {
  // App phase
  phase: 'login' | 'showfile' | 'welcome' | 'main'
  activePanel: 'home' | 'control' | 'player' | 'group' | 'customisation' | 'info' | 'effect' | 'hub' | 'keybind' | 'timecode' | 'showfile'
  ...
  showFiles: { id: string; name: string; description: string | null; lastSavedAt: string | null; createdAt: string; robloxPlaceId: string | null }[]
  hasUnsavedChanges: boolean

  // Actions
  setPhase: (phase: 'login' | 'showfile' | 'welcome' | 'main') => void
  setActivePanel: (panel: 'home' | 'control' | 'player' | 'group' | 'customisation' | 'info' | 'effect' | 'hub' | 'keybind' | 'timecode' | 'showfile') => void
  ...
  // ShowFile
  setActiveShowFile: (id: string | null) => void
  setShowFiles: (files: { id: string; name: string; description: string | null; lastSavedAt: string | null; createdAt: string; robloxPlaceId: string | null }[]) => void
  setHasUnsavedChanges: (v: boolean) => void
  setCurrentUser: (user: PlayerData) => void
  setHubEffects: (effects: HubEffect[]) => void
}
...
export const useBlurStore = create<BlurState>((set, get) => ({
  ...
  currentUser: { id: '', name: '', role: 'normal' as const },
  players: [],
  ...
  // ShowFile actions
  setActiveShowFile: (id) => set({ activeShowFileId: id }),
  setShowFiles: (files) => set({ showFiles: files }),
  setHasUnsavedChanges: (v) => set({ hasUnsavedChanges: v }),
  setCurrentUser: (user) => set({ currentUser: user }),
  setHubEffects: (effects) => set({ hubEffects: effects }),
}))
